            ...
            <tbody>
              {coinslist.map((i, idx) => (
                <tr className='border-y h-12' key={i.id} >
                  ...
                  <td className="marketcap bg-slate-800 text-center">{marked.includes(i.id) ? <img className='invert h-8' src={saved} onClick={() => (unsaveitem(i.id))} alt="" /> : <img className='invert h-8' src={save} onClick={() => { postitem(i.id); }} alt="" />}</td>
                </tr>
              ))
              }
